...
import com.e.tracker.xml.gpx.domain.Copyright
import com.e.tracker.xml.gpx.domain.Email
import com.e.tracker.xml.gpx.domain.Gpx
import com.e.tracker.xml.gpx.domain.Link
import com.e.tracker.xml.gpx.domain.Metadata
import com.e.tracker.xml.gpx.domain.Point
import com.e.tracker.xml.gpx.domain.Route
import com.e.tracker.xml.gpx.domain.RoutePoint
...
class GPXParser {
    ...
    fun parse(gpxUrl: String?, listener: GpxFetchedAndParsed?) {
        FetchAndParseGPXTask(gpxUrl!!, listener!!).execute()
    }
    ...
    private fun readGpx(parser: XmlPullParser): Gpx {
        ...
        while (loopMustContinue(parser.next())) {
            ...
            when (name) { TAG_METADATA -> builder.setMetadata(
                readMetadata(parser)
            )
                TAG_WAY_POINT -> wayPoints.add(
                    readWayPoint(
                        parser
                    )
                )
...
// to their respective "read" methods for processing. Otherwise, skips the tag.
    ...
    private fun readTrack(parser: XmlPullParser): Track {
        ...
        while (loopMustContinue(parser.next())) {
            ...
            val name = parser.name
            when (name) {
                GPXParser.Companion.TAG_NAME -> trackBuilder.setTrackName(
                    readName(parser)
                )
                GPXParser.Companion.TAG_SEGMENT -> segments.add(
                    readSegment(
                        parser
                    )
                )
                GPXParser.Companion.TAG_DESC -> trackBuilder.setTrackDesc(
                    readDesc(parser)
                )
                GPXParser.Companion.TAG_CMT -> trackBuilder.setTrackCmt(
                    readCmt(
                        parser
                    )
                )
                GPXParser.Companion.TAG_SRC -> trackBuilder.setTrackSrc(
                    readString(parser, GPXParser.Companion.TAG_SRC)
                )
                GPXParser.Companion.TAG_LINK -> trackBuilder.setTrackLink(
                    readLink(parser)
                )
                GPXParser.Companion.TAG_NUMBER -> trackBuilder.setTrackNumber(
                    readNumber(parser)
                )
                GPXParser.Companion.TAG_TYPE -> trackBuilder.setTrackType(
                    readString(parser, GPXParser.Companion.TAG_TYPE)
                )
                else -> skip(parser)
            }
        }
    ...
    private fun readLink(parser: XmlPullParser): Link {
        parser.require(
            XmlPullParser.START_TAG,
            GPXParser.Companion.namespace,
            GPXParser.Companion.TAG_LINK
        )
        val linkBuilder: Link.Builder = Link.Builder()
        linkBuilder.setLinkHref(
            parser.getAttributeValue(
                GPXParser.Companion.namespace,
                GPXParser.Companion.TAG_HREF
            )
        )
        while (loopMustContinue(parser.next())) {
            if (parser.eventType != XmlPullParser.START_TAG) {
            ...
            val name = parser.name
            when (name) {
                GPXParser.Companion.TAG_TEXT -> linkBuilder.setLinkText(
                    readString(parser, GPXParser.Companion.TAG_TEXT)
                )
                GPXParser.Companion.TAG_TYPE -> linkBuilder.setLinkType(
                    readString(parser, GPXParser.Companion.TAG_TYPE)
                )
                else -> skip(parser)
            }
    ...
    private fun readBounds(parser: XmlPullParser): Bounds {
        ...
        val bounds: Bounds = Bounds.Builder()
    ...
    private fun readSegment(parser: XmlPullParser): TrackSegment {
        val points: MutableList<TrackPoint> = ArrayList<TrackPoint>()
        ...
        while (loopMustContinue(parser.next())) {
            ...
            val name = parser.name
            if (GPXParser.Companion.TAG_TRACK_POINT == name) {
    ...
    private fun readRoute(parser: XmlPullParser): Route {
        ...
        while (loopMustContinue(parser.next())) {
            ...
            val name = parser.name
            when (name) {
                GPXParser.Companion.TAG_ROUTE_POINT -> points.add(
                    readRoutePoint(
                        parser
                    )
                )
                GPXParser.Companion.TAG_NAME -> routeBuilder.setRouteName(
                    readName(parser)
                )
                GPXParser.Companion.TAG_DESC -> routeBuilder.setRouteDesc(
                    readDesc(parser)
                )
                GPXParser.Companion.TAG_CMT -> routeBuilder.setRouteCmt(
                    readCmt(
                        parser
                    )
                )
                GPXParser.Companion.TAG_SRC -> routeBuilder.setRouteSrc(
                    readString(parser, GPXParser.Companion.TAG_SRC)
                )
                GPXParser.Companion.TAG_LINK -> routeBuilder.setRouteLink(
                    readLink(parser)
                )
                GPXParser.Companion.TAG_NUMBER -> routeBuilder.setRouteNumber(
                    readNumber(parser)
                )
                GPXParser.Companion.TAG_TYPE -> routeBuilder.setRouteType(
                    readString(parser, GPXParser.Companion.TAG_TYPE)
                )
                else -> skip(parser)
            }
    ...
    private fun readPoint(
        ...
        builder.setLatitude(
            java.lang.Double.valueOf(parser.getAttributeValue(namespace, TAG_LAT)))

        builder.setLongitude(
            java.lang.Double.valueOf(parser.getAttributeValue(namespace, TAG_LON))
        )
        ...
        while (loopMustContinue(parser.next())) {
            ...
            when (name) {
                ...
                TAG_ELEVATION -> builder.setElevation(
                    readElevation(parser)
                )
    ...
    private fun readMetadata(parser: XmlPullParser): Metadata {
        ...
        parser.require(
            XmlPullParser.START_TAG,
            GPXParser.Companion.namespace,
            GPXParser.Companion.TAG_METADATA
        )
        while (loopMustContinue(parser.next())) {
            if (parser.eventType != XmlPullParser.START_TAG) {
                continue
            ...
            val name = parser.name
            when (name) {
                GPXParser.Companion.TAG_NAME -> metadataBuilder.setName(
                    readName(
                        parser
                    )
                )
                GPXParser.Companion.TAG_DESC -> metadataBuilder.setDesc(
                    readDesc(
                        parser
                    )
                )
                GPXParser.Companion.TAG_AUTHOR -> metadataBuilder.setAuthor(
                    readAuthor(parser)
                )
                GPXParser.Companion.TAG_COPYRIGHT -> metadataBuilder.setCopyright(
                    readCopyright(parser)
                )
                GPXParser.Companion.TAG_LINK -> metadataBuilder.setLink(
                    readLink(
                        parser
                    )
                )
                GPXParser.Companion.TAG_TIME -> metadataBuilder.setTime(
                    readTime(
                        parser
                    )
                )
                GPXParser.Companion.TAG_KEYWORDS -> metadataBuilder.setKeywords(
                    readString(parser, GPXParser.Companion.TAG_KEYWORDS)
                )
                GPXParser.Companion.TAG_BOUNDS -> metadataBuilder.setBounds(
                    readBounds(parser)
                )
                GPXParser.Companion.TAG_EXTENSIONS -> skip(parser)
                else -> skip(parser)
            }
        }
        parser.require(
    ...
    private fun readAuthor(parser: XmlPullParser): Author {
        val authorBuilder: Author.Builder = Author.Builder()
        parser.require(
            XmlPullParser.START_TAG,
            GPXParser.Companion.namespace,
            GPXParser.Companion.TAG_AUTHOR
        )
        while (loopMustContinue(parser.next())) {
            if (parser.eventType != XmlPullParser.START_TAG) {
                continue
            ...
            val name = parser.name
            when (name) {
                GPXParser.Companion.TAG_NAME -> authorBuilder.setName(
                    readString(
                        parser,
                        GPXParser.Companion.TAG_NAME
                    )
                )
                GPXParser.Companion.TAG_EMAIL -> authorBuilder.setEmail(
                    readEmail(parser)
                )
                GPXParser.Companion.TAG_LINK -> authorBuilder.setLink(
                    readLink(
                        parser
                    )
                )
    ...
    private fun readEmail(parser: XmlPullParser): Email {
        parser.require(
            XmlPullParser.START_TAG,
            GPXParser.Companion.namespace,
            GPXParser.Companion.TAG_EMAIL
        )
        val emailBuilder: Email.Builder = Email.Builder()
        emailBuilder.setId(
            parser.getAttributeValue(
                GPXParser.Companion.namespace,
                GPXParser.Companion.TAG_ID
            )
        )
        emailBuilder.setDomain(
            parser.getAttributeValue(
                GPXParser.Companion.namespace,
                GPXParser.Companion.TAG_DOMAIN
            )
        )
    ...
    private fun readCopyright(parser: XmlPullParser): Copyright {
        parser.require(
            XmlPullParser.START_TAG,
            GPXParser.Companion.namespace,
            GPXParser.Companion.TAG_COPYRIGHT
        )
        val copyrightBuilder: Copyright.Builder = Copyright.Builder()
        copyrightBuilder.setAuthor(
            parser.getAttributeValue(
                GPXParser.Companion.namespace,
                GPXParser.Companion.TAG_AUTHOR
            )
        ...
        while (loopMustContinue(parser.next())) {
            ...
            val name = parser.name
            when (name) {
                GPXParser.Companion.TAG_YEAR -> copyrightBuilder.setYear(
                    readYear(parser)
                )
                GPXParser.Companion.TAG_LICENSE -> copyrightBuilder.setLicense(
                    readString(parser, GPXParser.Companion.TAG_LICENSE)
                )
                else -> skip(parser)
            }
        }
    ...
    private fun readWayPoint(parser: XmlPullParser): WayPoint {
        return readPoint(
            WayPoint.Builder(),
            parser,
            GPXParser.Companion.TAG_WAY_POINT
        ) as WayPoint
    }
    ...
    private fun readTrackPoint(parser: XmlPullParser): TrackPoint {
        return readPoint(
            TrackPoint.Builder(),
            parser,
            GPXParser.Companion.TAG_TRACK_POINT
        ) as TrackPoint
    }
    ...
    private fun readRoutePoint(parser: XmlPullParser): RoutePoint {
        return readPoint(
            RoutePoint.Builder(),
            parser,
            GPXParser.Companion.TAG_ROUTE_POINT
        ) as RoutePoint
    }

    @Throws(IOException::class, XmlPullParserException::class)
    private fun readName(parser: XmlPullParser): String {
        return readString(parser, GPXParser.Companion.TAG_NAME)
    }

    @Throws(IOException::class, XmlPullParserException::class)
    private fun readDesc(parser: XmlPullParser): String {
        return readString(parser, GPXParser.Companion.TAG_DESC)
    }

    @Throws(IOException::class, XmlPullParserException::class)
    private fun readType(parser: XmlPullParser): String {
        return readString(parser, GPXParser.Companion.TAG_TYPE)
    }

    @Throws(IOException::class, XmlPullParserException::class)
    private fun readCmt(parser: XmlPullParser): String {
        return readString(parser, GPXParser.Companion.TAG_CMT)
    }
    ...
    private fun readElevation(parser: XmlPullParser): Double {
        parser.require(
            XmlPullParser.START_TAG,
            GPXParser.Companion.namespace,
            GPXParser.Companion.TAG_ELEVATION
        )
    ...
    private fun readTime(parser: XmlPullParser): DateTime {
        ...
        //val time: LocalDateTime = ISODateTimeFormat.dateTimeParser().parseDateTime(readText(parser))
        //var time = "time"
        val time: DateTime = ISODateTimeFormat.dateTimeParser().parseDateTime(readText(parser))
